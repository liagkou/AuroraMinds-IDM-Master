...
public class PestoIdPImpl extends AbstractPestoIdP implements PestoIdP {

	public PestoIdPImpl(PestoDatabase database, List<IdentityProver> identityProvers, Map<String, MFAAuthenticator> authenticators, ServerCryptoModule cryptoModule, int rateLimit) throws SetupException {
	...
	public String authenticate(String username, byte[] cookie, long salt, byte[] signature, Policy policy) throws AuthenticationFailedException, OperationFailedException {
		...
		if(authenticated) {
			...
			} catch(PolicyUnfulfilledException | TokenGenerationException | OperationFailedException e) {
